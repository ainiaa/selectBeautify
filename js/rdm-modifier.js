...
function rdmFindIssueStatus() {
    var status;
    var statusList2 = $('.status');
    showError('statusList2:');
    showError(statusList2);
...
function rdmFindIssueStatus() {
    ...
    if (statusList.length > 0) {
        var children = statusList.eq(0).children('.value');
        if (children.length < 1) {
            showError("not found status info(children is wrong)!!");
        } else {
            status = children.html();
        }
    } else {
        showError("not found status info!!");
    }
...
var autoAssign = [
    {"status": "开发评估", "assignTo": "liujl 刘健林"},
    {"status": "交付测试", "assignTo": "gongl 龚亮"},
    {"status": "开发中", "assignTo": "gongl 龚亮"}
];
...
function getCurrentUser() {
    ...
    if (loggedas && loggedas.childNodes && loggedas.childNodes.length == 2) {//第一个为：“登录为” 第二个为所需的用户名
